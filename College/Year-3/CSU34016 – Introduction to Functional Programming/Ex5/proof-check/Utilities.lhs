...
\newpage
\section{List and Set Functions}

\subsection{Mapping over lists of pairs}
\begin{code}
mappair :: ((s1->t1),(s2->t2)) -> [(s1,s2)] -> [(t1,t2)]
mappair _ [] = []
mappair (f,g) ((x,y):pairs) = (f x,g y): mappair (f,g) pairs
\end{code}

\subsection{Converting list to all possible (ordered) pairs}
\begin{code}
pairup :: [a] -> [(a,a)]
pairup [] = []
pairup [_] = []
pairup (x:xs) = (map (pair x) xs) ++ pairup xs where pair x y = (x,y)
\end{code}
...
\begin{code}
alookup :: (Eq k, MonadFail m) => k -> [(k,d)] -> m (k,d)
...
\subsection{Instances: Functor, Applicative, Monad}
