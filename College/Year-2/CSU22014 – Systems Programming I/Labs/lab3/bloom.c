...
int bloom_lookup(struct bloom * this, char * item) {
	int hash_one = hash_string(item, BLOOM_HASH1, this -> size);
	int hash_two = hash_string(item, BLOOM_HASH2, this -> size);
	if(bitset_lookup(this -> bitset, hash_one) == 1 && bitset_lookup(this -> bitset, hash_two) == 1){
		return 1;
	}else{
		return 0;
	}	
...
void bloom_add(struct bloom * this, char * item) {
	int hash_one = hash_string(item, BLOOM_HASH1, this -> size);
	int hash_two = hash_string(item, BLOOM_HASH2, this -> size);
	bitset_add(this -> bitset, hash_one);
	bitset_add(this -> bitset, hash_two);
