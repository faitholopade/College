...
void insertionSort(int a[], int size) {
	for (int i = 1; i < size; i++) // element 0 sorted by default so start i at 1
	{
		int key = a[i]; // current elem
		int j = i - 1;
...
void bubbleSort(int a[], int size) {
	for (int i = 0; i < size; i++)
	{
		for (int j = 0; j < size - 1; j++) // last elements isn't being compared to anything sowe stop at size - 1
		{
			if (a[j] > a[j + 1])
			{
				int temp = a[j];
