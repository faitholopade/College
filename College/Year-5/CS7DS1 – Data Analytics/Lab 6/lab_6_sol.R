...
X <- model.matrix(latestPrice ~ livingAreaSqFt + numOfBedrooms, data = dat)
y <- dat$latestPrice
...
RSS        <- as.numeric(t(resid) %*% resid)  # Sum of squared residuals
p          <- ncol(X)   # number of beta parameters (excluding sigma)
df         <- n - p  # Degrees of freedom (n - p)
sigma2_hat <- RSS / df   # residual variance
sigma_hat  <- sqrt(sigma2_hat)  # residual standard error
...
varcov_beta <- sigma2_hat * XtX_inv
...
p_vals <- 2 * (1 - pt(abs(t_stats), df = df))
